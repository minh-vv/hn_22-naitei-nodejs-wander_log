...
const ItineraryDetail = () => {
  ...
  const handleSaveActivity = async (activityData) => {
    ...
    try {
      if (activityData.id) {
        await activityService.updateActivity(activityData.id, activityData, token);
      } else {
        await activityService.createActivity(activityData, token);
      }
      ...
      <div className={styles.container}>
        ...
        <div className={styles.heroSection}>
          <div className={styles.heroImageContainer}>
            <img
              src={itinerary.coverImage || 'https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=2670&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}
              alt={itinerary.title}
              ...
              <div className={styles.heroDetails}>
                ...
                <span className={styles.heroDetailItem}>
                  ...
                  {itinerary.startDate && itinerary.endDate ? 
                    `${(new Date(itinerary.endDate) - new Date(itinerary.startDate)) / (1000 * 60 * 60 * 24) + 1} days` : 
                    'Unknown'}
                </span>
                <span className={styles.heroDetailItem}>
                  <i className="ri-money-dollar-circle-line"></i>
                  {itinerary.budget ? `${itinerary.budget.toLocaleString('en-GB')} VND` : 'Unknown'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className={styles.authorSection}>
          <div className={styles.authorInfo}>
            <img
              src={itinerary.user?.avatar || 'https://readdy.ai/api/search-image?query=Professional%20headshot%20of%20a%20young%20Asian%20female%20traveler%20with%20a%20warm%20confident%20smile%2C%20wearing%20casual%20modern%20clothing%2C%20clean%20studio%20background%20with%20soft%20professional%20lighting&width=50&height=50&seq=author-profile&orientation=squarish'}
              alt={itinerary.user?.name || 'Author'}
              className={styles.authorAvatar}
            />
            <div>
              <div className={styles.authorNameContainer}>
                <h3 className={styles.authorName}>{itinerary.user?.name || 'Anonymous'}</h3>
              ...
              <p className={styles.createdAt}>
                  Created: {itinerary.createdAt ? new Date(itinerary.createdAt).toLocaleDateString() : 'Unknown'}
              </p>
            </div>
          </div>
            ...
            sortedDates.map((dateKey) => (
              <div key={dateKey} className={styles.dayCard}>
                <div className={styles.dayHeader}>
                  <div className={styles.dayNumber}>{calculateDayNumber(dateKey)}</div>
                  <div>
                    <h2 className={styles.dayTitle}>Day {calculateDayNumber(dateKey)}</h2>
                    <p className={styles.dayDate}>{formatDate(dateKey, 'dddd, DD MMMM YYYY')}</p>
                  </div>
                </div>
                ...
                <div className={styles.activitiesList}>
                  {groupedActivities[dateKey].map(activity => (
                    <div key={activity.id} className={styles.activityItem}>
                      <div className={styles.activityTime}>{activity.startTime}</div>
                      <div className={styles.activityContent}>
                        ...
                        <div className={styles.activityDetails}>
                          ...
                          <span className={styles.activityCost}>
                            {activity.cost ? `${activity.cost.toLocaleString('en-GB')} VND` : 'Unknown'}
                          </span>
                        </div>
                      </div>
                    </div>
